neoxr.create(async (m, { client, text }) => {
  let monsters = [
    { area: 1, name: "Goblin" },
    { area: 1, name: "Slime" },
    { area: 1, name: "Wolf" },
  ...
  let player = global.db.users.find(v => v.jid == m.sender)
...
//  let cds = `${DeTik(new Date - player.lasthunt)}`
 ...
 let __timers = (new Date - global.db.users.find(v => v.jid == m.sender).lasthunt)
 ...
 let timers = clockString(_timers)
  ...
  if (new Date - global.db.users.find(v => v.jid == m.sender).lasthunt > 1200000) {
